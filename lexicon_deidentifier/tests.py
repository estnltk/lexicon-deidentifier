...
class TestText(unittest.TestCase):
    ...
    def test(self):
        ...
        pii_dictionary = Dictionary(vocabulary=vocabulary, 
                                    look_ahead=2,
                                    lower=True,
                                    use_suffix_lemmatizer=False)
        t = Text(u'Tõnu Tamm alguses ja lõpus Tõnu', 
                 pii_dictionary=pii_dictionary)
        self.assertEquals(len(t.pii_texts), 2)
        self.assertTrue(u'tõnu tamm' in t.pii_texts)
        self.assertTrue(u'tõnu' in t.pii_texts)
        self.assertEquals(t.pii_starts, [0, 27])
        self.assertEquals(t.pii_ends, [9, 31])
